...
try:
    ...
    print(
        textcolors.warning + "Selection not available, using default." + textcolors.end
    )
...
if is_input:
    print(
        textcolors.blue + "Selection is input using standard mode.\n" + textcolors.end
    )
else:
    if is_wasapi:
        useloopback = True
...
for i in range(
    0, int(int(device_info["defaultSampleRate"]) / defaultframes * recordtime)
):
...
filename = (
    input("Save as [" + textcolors.blue + "out.wav" + textcolors.end + "]: ")
    or "out.wav"
)
